...
namespace ProjectAPI.Api.Application.Identity.Users.Register;
...
public class RegisterHandler : IRequestHandler<RegisterCommand, string>
{
    ...
    public async Task<string> Handle(RegisterCommand request, CancellationToken cancellationToken)
    {
        ...
        // outcome this endpoint can ever produce is PROSPECT.
        //
        // Nothing is assigned to the discriminator here: PROSPECT is a plain
        // User, and under TPH the CLR type already IS the stored value (see
        // ApplicationDbContext.HasDiscriminator). The role itself is granted
        // by AddToRoleAsync below, always normalized, so the guard above and
        // the write below cannot drift apart the way they once did when the
        // caller's raw, possibly-legacy string ("Notaire" instead of "NOTARY")
        // was persisted while authorization used the normalized form.
